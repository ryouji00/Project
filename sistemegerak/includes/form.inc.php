	...
	if(empty($category) || empty($worknama) || empty($placename) || empty($tripstart) || empty($tripend) || empty($timestart) || empty($timeend) || empty($pegawai)) {
		...
		if(!mysqli_stmt_prepare($stmt,$sql)){
			...
			if($resultCheck > 0){
				...
				$sql = "INSERT INTO destination ('category','workname','placename','tripstart','tripend','timestart','timeend','replaceofficer','staffid') VALUE ('$category','$worknama','$placename','$tripstart','$tripend','$timestart','$timeend','$pegawai','$staffid')";
				$stmt = mysqli_stmt_init($conn);
				if(!mysqli_stmt_prepare($stmt,$sql)){
